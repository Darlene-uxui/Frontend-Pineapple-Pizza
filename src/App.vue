<template>
  <div class="grid-container">
    ...
    <main>
      ...
      <PizzaListItem @delete-pizza="deletePizza" @update-pizza="updataPizza" v-for="pizza of pizzasArray"
        :key="pizza.id" :pizza-data="pizza" />
    </main>
    <aside>
      <h3>Meme Collection</h3>
      <img src="https://i.kym-cdn.com/photos/images/newsfeed/001/217/804/4d8.png">
      <img
        src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQ1-71_bQBedd1cGLMc0dCRdnRs2wO1_HncejxsOTS7nkP8B2ociGne8WQFcMV6kRbnTwI&usqp=CAU"
        alt="">
...
export default {
  ...
  methods: {
    ...
    async updataPizza(newPizzaData, pizzaData, openModal) {
      const response = await fetch(`http://localhost:3000/pizza/${pizzaData.id}`, {
        method: "PUT",
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({
          title: newPizzaData.title,
          description: newPizzaData.description,
          nickname: pizzaData.nickname,
          imageurl: newPizzaData.imageurl,
          id: pizzaData.id
        })
      });
      const data = await response.text();
